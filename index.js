  ...
  try {
   ...
   app.patch('/menu/:id', async (req, res) => {
    ...
    app.post('/create-payment-intent', async (req, res) => {
      const { price } = req.body; // Destructure price from request body
  
      if (!price || isNaN(price)) {
          return res.status(400).send({ error: 'Invalid price value' });
      }
  
      const amount = Math.round(price * 100); // Ensure price is properly multiplied and rounded
...
// payment krr por cart theke delete krr jnno
